...
|--------|------------|-----------|---------|
| `api.daytona.llmvault.dev` | A | Control VPS public IP | Daytona API + Dashboard |
| `dex.daytona.llmvault.dev` | A | Control VPS public IP | OIDC authentication |
| `*.preview.llmvault.dev` | A (wildcard) | Control VPS public IP | Primary preview proxy domain |
...
The **primary domain** (`*.preview.llmvault.dev`) uses a pre-provisioned wildcard cert via Cloudflare DNS challenge for maximum reliability. All **dynamic alias domains** use on-demand TLS with HTTP-01 challenge — no Cloudflare integration needed for those domains.
...
api_domain: "api.daytona.llmvault.dev"
dex_domain: "dex.daytona.llmvault.dev"
...
primary_preview_domain: "preview.llmvault.dev"
...
# Dex
dex_admin_email: "admin@llmvault.dev"
...
services:
  ...
  # 1. Static TLS for api.daytona.llmvault.dev + dex.daytona.llmvault.dev
  # 2. Wildcard TLS for *.preview.llmvault.dev (primary, Cloudflare DNS)
...
2. **Primary preview domain** — `*.preview.llmvault.dev` with Cloudflare DNS challenge wildcard cert (always reliable)
...
# =============================================================================
# TIER 2: PRIMARY PREVIEW DOMAIN — *.preview.llmvault.dev
...
3. [ ] Have a Cloudflare account with `llmvault.dev` zone
4. [ ] Have a Cloudflare API token with DNS:Edit for `llmvault.dev`
...
9. [ ] Create DNS records:
   - `api.daytona.llmvault.dev` A → control VPS IP
   - `dex.daytona.llmvault.dev` A → control VPS IP
   - `*.preview.llmvault.dev` A → control VPS IP
...
11. **Runners use the API's public domain** (`https://api.daytona.llmvault.dev/api`) not Docker hostnames — they're on separate machines.
