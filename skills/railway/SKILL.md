...
Manage Railway cloud infrastructure through a pre-authenticated GraphQL API proxy. All requests go to `${ZIRALOOP_RAILWAY_API_URL}` with no auth headers — the proxy handles authentication.
...
Every call is `curl -s -X POST ${ZIRALOOP_RAILWAY_API_URL} -H "Content-Type: application/json" -d '{"query": "...", "variables": {...}}'`. Always pipe through `jq`.
...
```bash
# BAD
curl -s -X POST ${ZIRALOOP_RAILWAY_API_URL} -H "Content-Type: application/json" -d '...'
...
curl -s -X POST ${ZIRALOOP_RAILWAY_API_URL} -H "Content-Type: application/json" \
...
```bash
SERVICE_ID=$(curl -s -X POST ${ZIRALOOP_RAILWAY_API_URL} -H "Content-Type: application/json" \
