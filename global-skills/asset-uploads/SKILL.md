---
name: asset-uploads
description: Use whenever you need a URL for a file produced in the sandbox. The platform provides HIVELOOP_DRIVE_UPLOAD_URL and UPLOAD_BEARER. Upload generated images, videos, audio, screenshots, charts, CSV/Excel exports, PDFs, zip bundles, reports, and PR/demo artifacts here, then share the returned file URL.
---

# Asset uploads

Upload files using `HIVELOOP_DRIVE_UPLOAD_URL` and `UPLOAD_BEARER`.
...
| Variable | Purpose |
|---|---|
| `HIVELOOP_DRIVE_UPLOAD_URL` | Asset upload folder for this sandbox/task |
| `UPLOAD_BEARER` | Bearer token for uploads |

```bash
test -n "$HIVELOOP_DRIVE_UPLOAD_URL" || { echo "HIVELOOP_DRIVE_UPLOAD_URL is not set" >&2; exit 1; }
...
url=$(
  curl -fsS -X PUT \
    ...
    "$HIVELOOP_DRIVE_UPLOAD_URL/output.png" \
  | jq -r .asset_url
)
printf '%s\n' "$url"
```
...
curl -fsS -X PUT \
  ...
  "$HIVELOOP_DRIVE_UPLOAD_URL/artifacts/metrics.csv"
```
...
- Use `HIVELOOP_DRIVE_UPLOAD_URL` exactly as provided.
