...
Every call is `curl -s -X POST ${HIVY_RAILWAY_API_URL} -H "Authorization: Bearer $HIVY_RAILWAY_API_KEY" -H "Content-Type: application/json" -d '{"query": "...", "variables": {...}}'`. Always pipe through `jq`.
...
```bash
curl -s -X POST ${HIVY_RAILWAY_API_URL} \
  -H "Authorization: Bearer $HIVY_RAILWAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "...", "variables": {...}}' \
  | jq 'if .errors then {error: .errors[0].message} else .data end'
```

Every request must include `Authorization: Bearer $HIVY_RAILWAY_API_KEY`.
...
```bash
# BAD
curl -s -X POST ${HIVY_RAILWAY_API_URL} -H "Authorization: Bearer $HIVY_RAILWAY_API_KEY" -H "Content-Type: application/json" -d '...'
...
curl -s -X POST ${HIVY_RAILWAY_API_URL} -H "Authorization: Bearer $HIVY_RAILWAY_API_KEY" -H "Content-Type: application/json" \
...
```bash
SERVICE_ID=$(curl -s -X POST ${HIVY_RAILWAY_API_URL} -H "Authorization: Bearer $HIVY_RAILWAY_API_KEY" -H "Content-Type: application/json" \
...
| Forgetting Authorization header | Always include `-H "Authorization: Bearer $HIVY_RAILWAY_API_KEY"` |
