...
# --- Check requirements ---
if [ -z "${ZIRALOOP_API_KEY:-}" ]; then
  echo "ERROR: Set ZIRALOOP_API_KEY environment variable"
  echo "  export ZIRALOOP_API_KEY=zira_sk_..."
  exit 1
fi
...
WARMUP=$(curl -s --max-time 180 -X POST "https://api.ziraloop.com/v1/agents/$FIRST_AGENT/conversations" \
  -H "Authorization: Bearer $ZIRALOOP_API_KEY" \
...
curl -s --max-time 15 -X POST "https://api.ziraloop.com/v1/conversations/$WARMUP_ID/messages" \
  -H "Authorization: Bearer $ZIRALOOP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"content": "Say ok."}' >/dev/null
echo "Waiting for warmup response..."
for i in $(seq 1 30); do
  EVENTS=$(curl -s --max-time 5 "https://api.ziraloop.com/v1/conversations/$WARMUP_ID/events" \
    -H "Authorization: Bearer $ZIRALOOP_API_KEY" | python3 -c "
import sys,json
