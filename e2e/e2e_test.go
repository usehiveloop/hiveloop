...
func (h *testHarness) createOrg(t *testing.T) model.Org {
	...
	org := model.Org{
		...
		ZitadelOrgID: fmt.Sprintf("ztdl-e2e-%s", uuid.New().String()[:8]),
		RateLimit:    10000,
		Active:       true,
	}
...
func (h *testHarness) storeCredential(t *testing.T, org model.Org, baseURL, authScheme, apiKey string) model.Credential {
	...
	body := fmt.Sprintf(`{"label":"e2e-test","base_url":%q,"auth_scheme":%q,"api_key":%q}`,
...
func requireOpenRouterKey(t *testing.T) string {
	...
	if key == "" {
		t.Skip("OPENROUTER_API_KEY not set — skipping live LLM test")
	}
	return key
}
