...
func TestCreateSpecialistSandbox(t *testing.T) {
	...
	if sb.EmployeeID == nil || *sb.EmployeeID != agent.ID {
		t.Error("employee_id should be set")
	}
...
func TestCreateSpecialistSandbox_InheritsEmployeeEnvWithSubagentOverrides(t *testing.T) {
	orch, provider, db := setupOrchestrator(t)
	org := createTestOrg(t, db)
	cred := createTestCred(t, db, org.ID)
	...
	sb, err := orch.CreateSpecialistSandbox(context.Background(), &subagent)
	if err != nil {
		t.Fatalf("CreateSpecialistSandbox: %v", err)
	}
	t.Cleanup(func() {
	...
	if _, ok := env["BRIDGE_SKIP"]; ok {
		t.Fatalf("BRIDGE_SKIP should not be inherited into specialist env")
	}
}

func TestCreateSpecialistSandbox_InheritsEmployeeResourcesForRepositoryClone(t *testing.T) {
	orch, provider, db := setupOrchestrator(t)
	org := createTestOrg(t, db)
	cred := createTestCred(t, db, org.ID)
	employee := createTestAgent(t, db, org.ID, cred.ID)
...
func TestCreateSpecialistSandbox_InheritsGitIdentityFromEmployee(t *testing.T) {
	orch, provider, db := setupOrchestrator(t)
	org := createTestOrg(t, db)
	cred := createTestCred(t, db, org.ID)
	employee := createTestAgent(t, db, org.ID, cred.ID)
