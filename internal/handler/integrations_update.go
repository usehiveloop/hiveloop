...
func (h *IntegrationHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if req.Credentials != nil {
		provider, found := h.nango.GetProvider(nangoProviderName(integ.Provider))
		if !found {
		...
		if err := h.nango.UpdateIntegration(r.Context(), nk, nangoReq); err != nil {
			logging.FromContext(r.Context()).ErrorContext(r.Context(), "nango integration update failed", "error", err, "integration_id", integ.ID)
		...
		if fetchErr != nil {
			logging.FromContext(r.Context()).WarnContext(r.Context(), "failed to fetch nango integration details for config rebuild", "error", fetchErr, "nango_key", nk)
		} else {
	...
	if len(updates) > 0 {
		if err := h.db.Model(&integ).Updates(updates).Error; err != nil {
			logging.FromContext(r.Context()).ErrorContext(r.Context(), "failed to update integration", "error", err, "integration_id", integ.ID)
	...
	logging.FromContext(r.Context()).InfoContext(r.Context(), "integration updated", "integration_id", integ.ID, "credentials_updated", req.Credentials != nil)
