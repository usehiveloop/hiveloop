...
func (h *IntegrationHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	var existing int64
	if err := h.db.Model(&model.Integration{}).
	...
	if err := h.nango.CreateIntegration(r.Context(), nangoReq); err != nil {
		logging.FromContext(r.Context()).ErrorContext(r.Context(), "nango integration creation failed", "error", err, "provider", req.Provider)
	...
	if err != nil {
		logging.FromContext(r.Context()).WarnContext(r.Context(), "failed to fetch nango integration details for config", "error", err, "nango_key", nk)
	} else {
	...
	if err := h.db.Create(&integ).Error; err != nil {
		logging.FromContext(r.Context()).ErrorContext(r.Context(), "failed to store integration", "error", err, "provider", req.Provider)
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to create integration"})
		return
	}

	logging.FromContext(r.Context()).InfoContext(r.Context(), "integration created", "integration_id", integ.ID, "provider", req.Provider)
