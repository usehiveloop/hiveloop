...
func (h *EmployeeHandler) StartSandboxUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	if ok, err := h.employeeHasActiveSlackProfile(ctx, org.ID, agentID); err != nil {
		log.ErrorContext(ctx, "load employee slack profile for sandbox upgrade", "error", err, "agent_id", agentID)
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to load employee profile"})
		return
	} else if !ok {
...
func (h *EmployeeHandler) employeeHasActiveSlackProfile(ctx context.Context, orgID, agentID uuid.UUID) (bool, error) {
	var count int64
