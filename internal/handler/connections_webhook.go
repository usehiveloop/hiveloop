...
// @Security BearerAuth
// @Router /v1/connections/{id}/webhook-configured [patch]
func (h *ConnectionHandler) MarkWebhookConfigured(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.db.Preload("Integration").Where("id = ?", connID).First(&conn).Error; err != nil {
