...
func TestConnectionHandler_MissingUserContext(t *testing.T) {
	db := connectTestDB(t)
	t.Cleanup(func() {
	...
	endpoints := []struct {
		...
		{http.MethodGet, "/v1/connections/" + uuid.New().String()},
		{http.MethodDelete, "/v1/connections/" + uuid.New().String()},
		{http.MethodPost, "/v1/integrations/" + uuid.New().String() + "/connect-session"},
		{http.MethodPost, "/v1/integrations/" + uuid.New().String() + "/connections"},
	}
