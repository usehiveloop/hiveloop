...
func TestIntegrationHandler_Get_Success(t *testing.T) {
	...
	rr := h.doRequest(t, http.MethodGet, "/v1/integrations/"+integ.ID.String(), nil, &user)
...
func TestIntegrationHandler_Get_NotFound(t *testing.T) {
	h := newIntegrationHarness(t, nil)
	user := createTestUser(t, h.db, fmt.Sprintf("admin-%s@test.com", uuid.New().String()[:8]))

	rr := h.doRequest(t, http.MethodGet, "/v1/integrations/"+uuid.New().String(), nil, &user)
...
func TestIntegrationHandler_Get_DeletedNotFound(t *testing.T) {
	h := newIntegrationHarness(t, nil)
	user := createTestUser(t, h.db, fmt.Sprintf("admin-%s@test.com", uuid.New().String()[:8]))
	integ := createTestIntegration(t, h.db, "github")
