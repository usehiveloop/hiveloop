...
func distinctProductionSessionIDs(t *testing.T, db interface {
	...
	if err := db.Raw(
		"SELECT DISTINCT session_id FROM employee_memory_events WHERE agent_id = ? AND sandbox_id = ? ORDER BY session_id",
		agentID, sandboxID,
