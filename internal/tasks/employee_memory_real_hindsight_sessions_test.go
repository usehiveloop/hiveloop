...
func productionMemorySessions() []productionSession {
	return []productionSession{
		...
		{user: "Daniel", text: "The Rust bridge release should run cargo check across the workspace before we publish the image.", tools: []string{"bash: cargo check --workspace", "bash: cargo test -p hivy-sandboxes-runtime"}, reply: "Passed cargo check workspace and hivy sandboxes runtime tests.", noise: []string{"ship"}},
		...
		{user: "Daniel", text: "When the bridge posts 'Sorry, something went wrong', first check the sandbox SQLite event log before guessing.", tools: []string{"bash: sqlite3 /app/data/hivy-sandboxes-runtime.db selected latest error events"}, reply: "Confirmed. Sandbox SQLite event log is the first stop for generic bridge errors.", noise: []string{"handy"}},
