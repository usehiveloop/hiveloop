...
func productionMemorySessions() []productionSession {
	return []productionSession{
		...
		{user: "Daniel", text: "Can you look at the hivy sandboxes runtime build? The sandbox booted but healthz refused the connection.", tools: []string{"bash: checked /proc/1/environ", "bash: ran hivy-sandboxes-runtime --help"}, reply: "The binary required a newer glibc than the image had. We need the Debian 13 runtime image for this build.", noise: []string{"classic", "ugh"}},
		...
		{user: "Kim", text: "The Slack bot answered but then failed two turns later. Can you find the actual error from the sandbox?", tools: []string{"bash: sqlite3 /app/data/hivy-sandboxes-runtime.db selected latest error events", "bash: checked process env"}, reply: "Found it. The model call hit HTTP 402 insufficient credits through the proxy, not a Slack delivery issue.", noise: []string{"ah", "makes sense"}},
		...
		{user: "Aisha", text: "The employee sandbox must use proxy.usehivy.com with a ptok token. If a raw provider API key appears in sandbox env, that's a release blocker.", tools: []string{"read_file: checked employee runtime compile env", "bash: verified sandbox env var names"}, reply: "Verified. Sandbox env uses HIVY_PROXY_API_KEY and no provider key.", noise: []string{"important"}},
		...
		{user: "Daniel", text: "The Rust runtime release should run cargo check across the workspace before we publish the image.", tools: []string{"bash: cargo check --workspace", "bash: cargo test -p hivy-sandboxes-runtime"}, reply: "Passed cargo check workspace and hivy sandboxes runtime tests.", noise: []string{"ship"}},
		...
		{user: "Daniel", text: "When the runtime posts 'Sorry, something went wrong', first check the sandbox SQLite event log before guessing.", tools: []string{"bash: sqlite3 /app/data/hivy-sandboxes-runtime.db selected latest error events"}, reply: "Confirmed. Sandbox SQLite event log is the first stop for generic runtime errors.", noise: []string{"handy"}},
