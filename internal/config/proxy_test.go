...
func TestProxyOpenAIBaseURL_DefaultsToProductionV1(t *testing.T) {
	cfg := &Config{}
	if got := cfg.ProxyOpenAIBaseURL(); got != "https://proxy.usehivy.com/v1/proxy/v1" {
...
func TestProxyOpenAIBaseURL_PreservesConfiguredScheme(t *testing.T) {
	...
	if got := cfg.ProxyOpenAIBaseURL(); got != "http://host.docker.internal:8080/v1/proxy/v1" {
		t.Fatalf("ProxyOpenAIBaseURL = %q", got)
	}
}

func TestProxyOpenAIBaseURL_DoesNotDuplicateProxyMount(t *testing.T) {
