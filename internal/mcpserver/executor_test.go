...
func TestExecuteAction_GraphQL_BuildLogs(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.NewEncoder(writer).Encode(map[string]any{"data": map[string]any{"buildLogs": []any{"line1"}}})
	}))
	defer server.Close()
...
func TestExecuteAction_GraphQL_Service(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.Unmarshal(bodyBytes, &captured)
		writer.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(writer).Encode(map[string]any{"data": map[string]any{"service": map[string]any{"name": "web"}}})
	}))
	defer server.Close()
...
func TestExecuteAction_GraphQL_Deployments_NestedInput(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.Unmarshal(bodyBytes, &captured)
		writer.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(writer).Encode(map[string]any{"data": map[string]any{"deployments": map[string]any{"edges": []any{}}}})
	}))
	defer server.Close()
...
func TestExecuteAction_GraphQL_Mutation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		_ = json.Unmarshal(bodyBytes, &captured)
		writer.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(writer).Encode(map[string]any{"data": map[string]any{"deploymentRestart": true}})
	}))
	defer server.Close()
...
func TestExecuteAction_GraphQLQuery_PreDefinedQuery(t *testing.T) {
	...
	// Should use the pre-defined query string, not auto-generated.
	...
	// Variables should be the raw params.
	...
	// Should NOT have body_mapping keys at top level.
...
func TestExecuteAction_GraphQLQuery_TakesPriorityOverAutoGenerated(t *testing.T) {
	...
	// Action has BOTH graphql_query AND graphql_field/body_mapping.
	// graphql_query should take priority.
	action := &catalog.ActionDef{
		DisplayName:    "Service",
		Access:         "read",
		ResponseSchema: "service",
	...
	// Should be the pre-defined query, not auto-generated.
	...
	// Should have variables, not inline args.
