...
func TestExecuteAction_GraphQLQuery_PreDefinedQuery(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		json.Unmarshal(bodyBytes, &capturedBodyMap)
		writer.Header().Set("Content-Type", "application/json")
		json.NewEncoder(writer).Encode(map[string]any{"data": map[string]any{"buildLogs": []any{}}})
	}))
	defer server.Close()
...
func TestExecuteAction_GraphQLQuery_TakesPriorityOverAutoGenerated(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		json.Unmarshal(bodyBytes, &capturedBodyMap)
		writer.Header().Set("Content-Type", "application/json")
		json.NewEncoder(writer).Encode(map[string]any{"data": map[string]any{"service": map[string]any{}}})
	}))
	defer server.Close()
