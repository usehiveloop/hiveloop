...
// on a tombstoned row. Preloads Connection so SourceKind() can
...
func buildConnector(src *ragmodel.RAGSource, deps *Deps) (interfaces.Connector, error) {
	...
	c, err := factory(src, interfaces.BuildDeps{Nango: deps.Nango, Spider: deps.Spider, DB: deps.DB, KMS: deps.KMS})
	if err != nil {
