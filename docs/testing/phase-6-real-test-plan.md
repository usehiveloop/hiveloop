...
# Bridge
BRIDGE_BASE_IMAGE_PREFIX=llmvault-bridge-0-10-0
BRIDGE_HOST=llmvault.outray.app
...
# https://llmvault.outray.app must point to localhost:8080
...
for sb in items:
    if sb.get('name','').startswith('llmv-'):
        print(f'  {sb[\"name\"]}  id={sb[\"id\"]}  state={sb[\"state\"]}')
        print(f'    DELETE: curl -X DELETE \"{sb.get(\"id\")}\"')
print(f'Total llmv sandboxes: {sum(1 for s in items if s.get(\"name\",\"\").startswith(\"llmv-\"))}')
"
...
# Clean local DB
PGPASSWORD=localdev psql -h localhost -p 5433 -U llmvault -d llmvault -c \
...
1. Create a Daytona sandbox from the `llmvault-bridge-0-10-0-small` snapshot
...
```bash
BRIDGE_URL=$(PGPASSWORD=localdev psql -h localhost -p 5433 -U llmvault -d llmvault -t -A -c \
...
- `provider.base_url` = `https://llmvault.outray.app/v1/proxy` (our proxy)
- `provider.provider_type` = `anthropic`
- `provider.api_key` starts with `ptok_` (minted proxy token)
- MCP server `llmvault` is configured
...
PGPASSWORD=localdev psql -h localhost -p 5433 -U llmvault -d llmvault -t -A -c \
...
SANDBOX_KEY=$(grep "^SANDBOX_PROVIDER_KEY=" .env | cut -d= -f2- | tr -d '[:space:]')
SANDBOX_URL=$(grep "^SANDBOX_PROVIDER_URL=" .env | cut -d= -f2- | tr -d '[:space:]')
EXTERNAL_ID=$(PGPASSWORD=localdev psql -h localhost -p 5433 -U llmvault -d llmvault -t -A -c \
