# ZiraLoop — Competitive Landscape

---

## Market Position

ZiraLoop occupies a new niche: **LLM Credential Management & Proxy**. No existing product does exactly what ZiraLoop does. The "competition" is either:
...
| Capability | ZiraLoop | Build In-House | Generic API Gateway (Kong/Apigee) | LLM Gateway (LiteLLM/Portkey) | Secret Manager (Vault/AWS SM) |
|---|---|---|---|---|---|
...
**ZiraLoop positioning vs. in-house**:
"Building secure LLM key management properly takes 3-6 months of senior engineering time. Most teams take shortcuts — and those shortcuts become liabilities. ZiraLoop gives you enterprise-grade security in hours, so your engineers can build your actual product."
...
**ZiraLoop positioning vs. API gateways**:
"API gateways handle traffic management. ZiraLoop handles credential custody. They solve different problems at different layers. Use an API gateway in front of ZiraLoop if you want both routing AND secure key management."
...
ZiraLoop is built for **your customers' keys** — to help you securely store, manage, and proxy their credentials.

**Complementary, not competitive**:
ZiraLoop + LiteLLM/Portkey can work together. ZiraLoop handles credential storage and proxy. The LLM gateway handles routing, observability, and cost management. The architecture: Customer → ZiraLoop (credential proxy) → LLM Gateway (routing/observability) → LLM Provider.

**ZiraLoop positioning vs. LLM gateways**:
"LLM gateways help you manage your own LLM costs and routing. ZiraLoop helps you manage your customers' LLM credentials securely. Different problems, complementary solutions."
...
**Key distinction**:
Secret managers are storage. ZiraLoop is storage + proxy + token management + auth abstraction. ZiraLoop actually uses Vault Transit under the hood — and adds the LLM-specific layers on top.

**ZiraLoop positioning vs. secret managers**:
"We use HashiCorp Vault under the hood for KMS. Then we add everything else: the streaming proxy, scoped tokens, auth scheme abstraction, multi-tier caching, and instant revocation. Vault stores secrets. ZiraLoop operationalizes LLM credentials."
...
9. **Brand**: "ZiraLoop" becomes the default answer to "how do I handle customer LLM keys?"
...
1. **ziraloop.com/compare/build-vs-buy** — "ZiraLoop vs. Building LLM Key Management In-House"
2. **ziraloop.com/compare/api-gateways** — "ZiraLoop vs. API Gateways (Kong, Apigee)"
3. **ziraloop.com/compare/llm-gateways** — "ZiraLoop vs. LLM Gateways (LiteLLM, Portkey)"
4. **ziraloop.com/compare/secret-managers** — "ZiraLoop vs. Secret Managers (Vault, AWS SM)"

Each page:
- Honest comparison (acknowledge where the alternative is better)
- Clear differentiation (what ZiraLoop does that they don't)
- "Better together" positioning where applicable (ZiraLoop + LiteLLM)
- CTA: "Try ZiraLoop free" and "Read the architecture docs"
