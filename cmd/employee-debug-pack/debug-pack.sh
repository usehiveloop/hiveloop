...

{
  ...
  missing=0
  for key in RUNTIME_SECRET SLACK_BOT_TOKEN SLACK_APP_TOKEN HIVELOOP_PROXY_API_KEY AGENT_MODEL AGENT_BASE_URL AGENT_API_KEY_ENV EMPLOYEE_ID CLOUD_CONTROL_PLANE_URL BRIDGE_API_KEY UPLOAD_BEARER WORKSPACE_ROOT DB_PATH RUNTIME_BIND_ADDR HIVELOOP_SANDBOX_ID HIVELOOP_ORG_ID HIVELOOP_AGENT_ID HIVELOOP_DRIVE_UPLOAD_URL; do
    eval "value=\${$key:-}"
    if [ -z "$value" ]; then
      echo "- $key"
      missing=1
