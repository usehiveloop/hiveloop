...
const hermesImageRepo = "ghcr.io/usehiveloop/hermes"

func runHermes(ctx context.Context, args []string) {
	fs := flag.NewFlagSet("hermes", flag.ExitOnError)
	version := fs.String("version", "", "Tag of usehiveloop/hermes already published to GHCR (required, e.g. v0.0.1)")
	...
	if err := registerHermesSnapshots(ctx, *version, targetSizes); err != nil {
		log.Fatalf("error: %v", err)
	}
	log.Println("Done.")
}

func registerHermesSnapshots(ctx context.Context, version string, targetSizes []string) error {
	cleanVersion := strings.TrimPrefix(version, "v")
	dashedVersion := strings.ReplaceAll(cleanVersion, ".", "-")
	imageRef := fmt.Sprintf("%s:%s", hermesImageRepo, cleanVersion)
	...
	for _, sizeName := range targetSizes {
		...
		name := hermesSnapshotName(dashedVersion, size.Name)
...
func hermesSnapshotName(dashedVersion, size string) string {
	return fmt.Sprintf("hiveloop-hermes-%s-%s-v1", dashedVersion, size)
}
