...
1. **Link table + agent-initiated linking**. Add an `agent_conversation_resource_links` many-to-many table. Give the agent a `hivy_link_resource(connection_id, resource_type, resource_id)` tool that inserts a row tying the current conversation to a future resource key. The executor's lookup does a two-step check: primary resource key first, then fall back to the link table.
