...
"The webhook arrived at 14:32. It was a `pull_request.opened` event on connection X. The dispatcher found 2 matching triggers. Trigger A was skipped because condition 0 (sender.login not_equals) failed — the sender was hivy-bot[bot] which matches the exclusion filter. Trigger B fired normally. So the user's trigger is correctly firing only for non-bot users. The issue is that the user expected Trigger A to fire, but Trigger A is configured to exclude bots..."
