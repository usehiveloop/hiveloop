...
| 5 | `TestDispatch_IssueComment_MentionOnIssue` | Two conditions with `match: all`: `comment.body contains "@hivy"` AND `issue.pull_request not_exists`. Exercises both operators and `{{$refs.x}}` mustache substitution in instructions. |
...
PASS
ok  	github.com/usehivy/hivy/internal/trigger/dispatch	1.493s
```
