use agent::specialists::SpecialistEvent;
...
use crate::specialist_callback_payload::{
...
pub struct SpecialistCallbackRequest {
    pub task_id: String,
...
#[cfg_attr(feature = "openapi", derive(utoipa::ToSchema))]
pub struct SpecialistCallbackResponse {
    pub accepted: bool,
    pub duplicate: bool,
...
fn capture_failure(
    ...
    let sentry_error = anyhow::anyhow!("specialist callback {phase}: {status}");
    sentry::with_scope(
        |scope| {
            ...
            scope.set_tag("specialist.operation", "callback");
            scope.set_tag("specialist.phase", phase);
            scope.set_tag("http.status_code", status.as_u16().to_string());
            set_non_empty_tag(scope, "specialist.task_id", &request.task_id);
            set_non_empty_tag(scope, "specialist_id", &request.specialist_id);
            set_non_empty_tag(scope, "specialist.event_type", &request.event_type);
            set_non_empty_tag(scope, "specialist.event_id", &request.event_id);
...
#[cfg_attr(feature = "openapi", utoipa::path(
    post,
    path = "/gateway/specialists/callback",
    request_body = SpecialistCallbackRequest,
    responses(
        (status = 202, description = "Callback accepted", body = SpecialistCallbackResponse),
        (status = 200, description = "Duplicate callback ignored", body = SpecialistCallbackResponse),
...
pub async fn post_specialist_callback(
    State(state): State<ApiState>,
    Json(request): Json<SpecialistCallbackRequest>,
) -> Result<(StatusCode, Json<SpecialistCallbackResponse>), (StatusCode, String)> {
...
async fn persist_and_deliver(
    ...
    if matches!(status, Some(SessionStatus::Errored)) {
        capture_failure(
            ...
            request,
            StatusCode::INTERNAL_SERVER_ERROR,
            "specialist reported an error event",
        );
    }
    ...
    if let Some(index) = state.specialist_task_index.as_ref() {
        index
            .append_event(
                &request.task_id,
                SpecialistEvent {
...
async fn deliver_http(
    ...
    http_gateway
        .broker
        .publish(&stream_id, "specialist_event", payload)
        .await;
    Ok(())
}
