...
cleanup() {
  "$DOCKER_BIN" logs "$NAME" >/tmp/hivy-sandboxes-runtime-smoke.log 2>&1 || true
  "$DOCKER_BIN" logs "$AUTH_NAME" >/tmp/hivy-sandboxes-runtime-auth-smoke.log 2>&1 || true
...
"$DOCKER_BIN" run -d \
  ...
  -e HIVY_GIT_CREDENTIALS_URL="http://127.0.0.1:18081/git-credentials" \
  "$IMAGE" \
  -c 'git config --system user.name "$HIVY_GIT_USERNAME"; git config --system user.email "$HIVY_GIT_EMAIL"; sleep 300' >/tmp/hivy-sandboxes-runtime-auth-container-id
