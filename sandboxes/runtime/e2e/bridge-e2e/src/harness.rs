...
impl TestHarness {
    ...
    /// Start the harness with codedb MCP server enabled.
    ///
    /// Requires `FIREWORKS_API_KEY` env var. Bridge is started with
    /// `BRIDGE_CODEDB_ENABLED=true` so built-in Grep/Read/Glob are replaced
    /// by codedb MCP tools. The codedb binary must be available.
    pub async fn start_with_codedb(codedb_binary: &str) -> Result<Self> {
        let fireworks_key = std::env::var("FIREWORKS_API_KEY")
            .context("FIREWORKS_API_KEY environment variable not set")?;
        ...
        // Use the bridge workspace itself as the codebase for codedb to index
        let codedb_workspace = workspace_root.clone();

        // 3. Start bridge with codedb enabled
        let bridge_port = Self::find_free_port()?;
        let bridge_listen_addr = format!("127.0.0.1:{}", bridge_port);
        let bridge_base_url = format!("http://127.0.0.1:{}", bridge_port);

        let bridge_stdout_log = std::fs::File::create(
            std::env::temp_dir().join(format!("bridge-e2e-codedb-stdout-{}.log", bridge_port)),
        )
        .unwrap_or_else(|_| std::fs::File::create("/dev/null").unwrap());
        let bridge_stderr_log = std::fs::File::create(
            std::env::temp_dir().join(format!("bridge-e2e-codedb-stderr-{}.log", bridge_port)),
        )
        .unwrap_or_else(|_| std::fs::File::create("/dev/null").unwrap());

        eprintln!(
            "[harness] Bridge codedb logs: stdout={}/bridge-e2e-codedb-stdout-{}.log stderr={}/bridge-e2e-codedb-stderr-{}.log",
            std::env::temp_dir().display(), bridge_port,
            std::env::temp_dir().display(), bridge_port,
        );
        ...
        bridge_command
            ...
            .env("BRIDGE_LISTEN_ADDR", &bridge_listen_addr)
            .env("BRIDGE_LOG_LEVEL", "debug")
            .env("BRIDGE_CODEDB_ENABLED", "true")
            .env("BRIDGE_CODEDB_BINARY", codedb_binary)
            .env(
                "BRIDGE_WEBHOOK_URL",
                format!("{}/webhooks/receive", cp_base_url),
            )
            .current_dir(&codedb_workspace)
            .stdout(Stdio::from(bridge_stdout_log))
            .stderr(Stdio::from(bridge_stderr_log));
        ...
        tracing::info!(port = bridge_port, "bridge process started (codedb mode)");
        ...
        // Push a codedb agent directly (not from mock CP fixtures)
        let codedb_agent = serde_json::json!({
            "id": "codedb-agent",
            "name": "CodeDB Test Agent",
            "system_prompt": "You are a coding assistant. Use the available tools to search and read code. When asked about code, use the codedb tools (codedb_search, codedb_read, codedb_outline, codedb_symbol, codedb_tree, codedb_word) to find and examine files.",
            "provider": {
        ...
        harness.push_agent_to_bridge(&codedb_agent).await?;

        // Wait for agent to be loaded (codedb MCP connection takes a moment)
