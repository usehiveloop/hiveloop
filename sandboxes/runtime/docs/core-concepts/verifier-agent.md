...
> **Wire-format note:** `provider` is `"open_ai"` (with an underscore), matching the main agent's `ProviderType::OpenAI` wire format. The natural `"openai"` is **not** accepted — bridge's serde rename keeps both enums consistent. See [the OpenAI variant in `crates/core/src/provider.rs`](https://github.com/usehiveloop/hiveloop/blob/main/sandboxes/runtime/crates/core/src/provider.rs) for the source of truth.
