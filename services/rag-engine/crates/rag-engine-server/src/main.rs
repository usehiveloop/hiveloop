...
use rag_engine_server::{
    ...
    shutdown_signal, telemetry, timeout_layer, AppState, Config, LimitsConfig, RagEngineService,
    ShutdownConfig, StateLimits, GRPC_SERVICE_NAME,
};
...
async fn run(config: Config) -> anyhow::Result<()> {
    ...
    // 2F: batch + idempotency limits consumed by AppState handlers.
    let state_limits = StateLimits::from_env();
    ...
    // 2H: transport-layer limits consumed by the tower middleware stack.
    let limits = LimitsConfig::from_env();
    ...
    // lifecycle (2H) + service (2F): tonic's built-in `max_decoding_message_size`
    // catches oversize payloads that slip past the content-length check in
    // `body_size_limit_layer` — HTTP/2 streaming requests without a
    // content-length header arrive here, and the protobuf decoder raises
    // `RESOURCE_EXHAUSTED` when a single message exceeds the configured
    // limit. Apply the limit on the base `RagEngineServer` *before* wrapping
    // in the auth interceptor. `RagEngineService::new(app_state)` wires 2F's
    // stateful handlers.
    let rag_service_inner = RagEngineServer::new(RagEngineService::new(app_state))
